...
export default function NewsSendPage() {
  ...
  const handleForceRecalc = async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/news-new/generate-matches', { method: 'POST' });
  ...
  const handleSendToday = async () => {
    ...
    try {
      const res = await fetch('/api/news-new/send-today', {
        method: 'POST',
      });
  ...
  const fetchTargets = async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/news-new/get-latest-matches');
      const data = await res.json();
      setTargets(data);
      ...
      if (uniqueIds.length > 0) {
        const articleRes = await fetch(`/api/news-new/get-articles-by-ids`, {
          method: 'POST',
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <h1 className="text-xl font-bold mb-4">本日配信予定のマッチング確認　※開発中</h1>

      <div className="flex gap-4 mb-4">
        <Link
  href="/admin-dashboard/news-new/new_matches"
  className="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded inline-block"
>
  🆕 新着記事 × 配信対象者 を確認する
</Link>
