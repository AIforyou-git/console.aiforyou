  ...
  useEffect(() => {
    const fetchArticles = async () => {
      ...
      if (clientData?.industry) {
        ...
        query = query.or(
          `structured_title.ilike.*${safeIndustry}*,
           structured_summary_extract.ilike.*${safeIndustry}*`
        );
      }

      // ✅ UI上のキーワードフィルター（全構造フィールド対象）
      if (keyword === "お気に入り") {
        ...
        query = query.or(
          `structured_title.ilike.*${safeKeyword}*,
           structured_summary_extract.ilike.*${safeKeyword}*,
           structured_agency.ilike.*${safeKeyword}*,
           structured_prefecture.ilike.*${safeKeyword}*`
        );
      }
